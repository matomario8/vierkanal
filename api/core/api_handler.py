from flask import Flask, Blueprint, jsonify, request, current_app, abort, render_template
...
imgboard = Blueprint("imgboard", __name__)
...
@imgboard.errorhandler(500)
def internal_server_error(e):
    return render_template("500.html"), 500

@imgboard.errorhandler(400)
...
@imgboard.route("/board/new", methods=["POST"])
def create_board():
...
@imgboard.route("/board/<string:board_id>/thread/<int:thread_id>/newreply", methods=["POST"])
def create_reply():
    model = "REPLY"
    response = _handle_post_request(model)
    return response

@imgboard.route("/board/<string:board_id>/reply/<int:reply_id>", methods=["GET"])
def get_reply():
    model = "REPLY"
    board_id = _validate_input(board_id)
...
@imgboard.route("/board/<string:board_id>/thread/<int:threadId>/replies", methods=["GET"])
def get_replies():
    """
...
@imgboard.route("/board/<string:board_id>/reply/<int:reply_id>/delete", methods=["DELETE"])
def delete_reply():
    pass

@imgboard.route("/board/<string:board_id>/thread/<int:post_id>", methods=["GET"])
...
@imgboard.route("/board/<string:board_id>/threads", methods=["GET"])
...
@imgboard.route("/board/<string:board_id>/newthread", methods=["POST"])
...
@imgboard.route("/board/<string:board_id>/thread/<int:thread_id>/delete")
def delete_thread():
    pass

@imgboard.route("/board/<string:board_name>/getnextid", methods=["GET"])
def get_next_post_id():
    pass

if Config.CONFIG is not None:
    _init_boards(Config.CONFIG["database"]["boards"])
